...
Корутину с `authorize` можно отменить штатным способом. Временные сетевые ошибки
отдельных poll-запросов повторяются с серверным интервалом до истечения срока
действия `device_code`.
...
Для потокового воспроизведения `audioRange(url, start, length, onHeaders, onBytes)`
читает диапазон ранее полученной прямой ссылки через настроенный ЯМ-прокси.
`onHeaders` получает проверенные границы `AudioRange`, `onBytes` — позицию в
полном файле и временный буфер байтов. Колбэки выполняются на IO-потоке;
буфер нужно обработать до возврата из колбэка. Отмена корутины отменяет HTTP Call.
Если сервер игнорирует Range и отвечает 200, передаётся полный файл с offset=0.
Ответ с неизвестным общим размером отклоняется.
